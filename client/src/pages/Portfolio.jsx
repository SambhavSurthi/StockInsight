...
const Portfolio = () => {
  ...
  const fetchPrices = async () => {
    ...
    try {
      ...
      const testRes = await fetch(API_ENDPOINTS.PORTFOLIO.BASE, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const res = await fetch(API_ENDPOINTS.PORTFOLIO.BULK_DELETE, {
        method: 'POST',
        headers: {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success(data.message || 'Removed from portfolio');
  ...
  const handleExport = () => {
    ...
    filteredCompanies.forEach((c) => {
      (prices[c.screenerId] || []).forEach((r) => allDates.add(r.dateLabel));
    });
    const dateHeaders = Array.from(allDates);

    const rows = filteredCompanies.map((c) => {
      const rowPrices = prices[c.screenerId] || [];
      const map = Object.fromEntries(rowPrices.map((r) => [r.dateLabel, r.price]));
      return [c.name, ...dateHeaders.map((d) => map[d] ?? '')];
    });

    const headerLine = ['Company', ...dateHeaders];
    const csvLines = [headerLine, ...rows].map((line) =>
      line
        .map((cell) => {
    ...
    link.href = url;
    link.setAttribute('download', 'portfolio_prices.csv');
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-4">
      <div className="flex items-center justify-between gap-2">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground sm:text-sm">
            All companies you have added to your portfolio.
          </p>
        </div>
      ...
      {editMode && (
        <div className="flex items-center justify-between rounded-lg border bg-muted/40 px-3 py-2 text-xs sm:text-sm">
      ...
      {comparisonMode && (
        <div className="flex items-center justify-between rounded-lg border bg-primary/10 px-3 py-2 text-xs sm:text-sm">
          <span className="text-muted-foreground">
            {comparisonSelectedIds.length} selected (Select 2-3 companies)
          </span>
          <button
            type="button"
            onClick={() => {
              if (comparisonSelectedIds.length < 2 || comparisonSelectedIds.length > 3) {
                toast.error('Please select 2-3 companies to compare');
                return;
              }
              const selectedCompanies = filteredCompanies.filter((c) =>
                comparisonSelectedIds.includes(c.screenerId)
              );
              const companiesData = selectedCompanies.map((c) => ({
                screenerId: c.screenerId,
                name: c.name,
              }));
              navigate('/compare', {
                state: { companies: companiesData, source: 'portfolio' },
              });
            }}
            disabled={comparisonSelectedIds.length < 2 || comparisonSelectedIds.length > 3}
            className="inline-flex items-center justify-center rounded-full bg-primary px-3 py-1 text-xs font-medium text-primary-foreground shadow hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed sm:text-sm"
          >
            Compare Selected
          </button>
        </div>
      ...
      {loadingList ? (
        ...
        <PortfolioTable
          ...
          onCompanyClick={(company) =>
            navigate(`/company/${company.screenerId}?name=${encodeURIComponent(company.name)}&from=portfolio`)
          }
        />
      )}
    </div>
